...
![versionName](https://img.shields.io/badge/versionName-0.1.14-blue)
...
| 문서 | 내용 |
|------|------|
| [CHANGELOG](docs/CHANGELOG.md) | 버전별 변경 이력 (v0.1.0 ~ v0.1.14) |
...
**현재 버전** — `0.1.14` (versionCode `28`) / 0.1.13/27 프로덕션 심사는 취소 → **0.1.14/28 = 출시 준비 종합 빌드** (백엔드 배포 완료, Android Play 업로드 대기). 0.1.14 = 실기기 제보 2버그 근본수정(빈 운동계획 = R8 keep 갭, 완료 토글 해제 보존 = 수동 우선) + 4-에이전트 전수감사 출시차단 해소 + 재발방지 (PR #122). 직전 0.1.13 = 코드베이스 리팩토링(내부 품질, 사용자 영향 없음)
