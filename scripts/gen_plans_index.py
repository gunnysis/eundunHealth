...
def render_readme_v2(
    ...
    lines.append("4. **(같은 commit) 참조 리다이렉트**: `git rm` 한 페어를 참조하던 곳이 있으면 함께 고친다 — 길잡이 참조는 경로를 ledger 로 **교체**, 이력 기록(CHANGELOG 의 \"Added\")은 원문을 두고 **리다이렉트를 병기**한다. 빠뜨리면 링크가 조용히 죽는다(2026-09-01 도입 시점 26건 적체). 검사: `bash scripts/check-plans-links.sh`")
    lines.append("   > **문서만이 아니다.** `# 설계: docs/plans/...` 주석은 워크플로·스크립트·Kotlin 소스에도 달리고, 오히려 그 근거를 가장 자주 읽게 되는 자리다. 가드가 `.md` 만 보던 동안 그쪽에 10건이 죽어 있었다(2026-09-02 실측). 지금은 추적 파일 전체를 검사한다.")
