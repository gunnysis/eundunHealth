...
- **alembic/ pre-existing 16건 위반** (I001/UP007/UP035 — typing union 모던 syntax): 본 plan 외. backend.yml 의 ruff scope (`app/ tests/`) 에 alembic 미포함이라 본 PR CI fail 위험 없음. 별도 chore PR 에서 `cd backend && .venv\Scripts\ruff.exe check --fix alembic/` 으로 일괄 fix 가능 — Task 5 실행 중 발견 (2026-06-02).
