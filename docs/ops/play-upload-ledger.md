...
2. Play Console 에 AAB **업로드 성공** 직후, 위 `LAST_UPLOADED_VERSION_CODE=` 값을 방금 올린 versionCode 로 갱신하고 이력 표에 행을 추가한다(같은 커밋 권장).
...
Play Developer Publishing API(서비스 계정 JSON)를 도입하면 가드가 원장 대신 Play 에서
직접 최고 versionCode 를 조회해 수동 갱신 의존을 제거할 수 있다
(`docs/ops/play-store-release.md` §7 옵션 2 의 service account 와 동일 자격). v1.0 자동 배포 시 검토.
